...
class SimulateIrregular(Task):
  ...
  def reset(self, env):
    ...
    zone_size = tuple([0.3,0.3,0.05])
    zone_pose = ([0.5,0,zone_size[2]/2],[0,0,np.pi/2,0])
    ...
    for object_id, _ in object_ids:
      ...
      boundaries = p.getAABB(object_id) # NOTE THAT IN PYBULLET, THE BBOX OF AN OBJECT IS PADDED WITH 2*(+0.003) IN X-, Y-, AND Z- DIMENSIONS
      lwh = np.array(boundaries[1])-np.array(boundaries[0])
      true_pose = ([0.5-lwh[0]/2,0.0-lwh[1]/2,lwh[2]/2],[0,0,np.pi*2,0])
      true_poses.append(true_pose)


    

  def reset_irr(self, env, obj_format_filename=None):
    super().reset(env)
    # Add container box.
    zone_size = self.get_random_size(0.05, 0.3, 0.05, 0.3, 0.05, 0.05)
    zone_pose = self.get_random_pose(env, zone_size)
    # Set a Fixed Bin
    zone_size = tuple([0.34, 0.34, 0.30])
    zone_pose = ([0.5,0,zone_size[2]/2],[0,0,np.pi/2,0])
    container_template = 'assets/container/container-template.urdf'
    half = np.float32(zone_size) / 2
    replace = {'DIM': zone_size, 'HALF': half}
  ...
  def add_an_object(self, env, obj_format_filename, target_location, color=None):
    super().reset(env)
    # Add objects in container.
    if len(self.goals) == 0:
      object_points = {}
      ...
      object_points = self.goals[0][6][0]
      object_ids = self.goals[0][0]
      object_volumes = []
      true_poses = self.goals[0][2]
    object_template = 'assets/packOBJ/pack_obj_template.urdf'
    for _ in range(1):
      urdf = self.fill_template(object_template, [], obj_filename=obj_format_filename)
      obj_id = env.add_object(urdf, target_location)
      os.remove(urdf)
      object_ids.append((obj_id, (0, None)))
      object_points[obj_id] = self.get_object_points(obj_id)
      boundaries = p.getAABB(obj_id)
      lwh = np.array(boundaries[1])-np.array(boundaries[0])
      start_location = target_location
      start_location[0][2] += 0.02
      true_pose = p.getBasePositionAndOrientation(obj_id)
      true_poses.append(true_pose)
      p.resetBasePositionAndOrientation(obj_id, start_location[0], start_location[1])
