...
class Environment():
  ...
  def reset_packing(self, task, obj_name=None):
    """Performs common reset functionality for all supported tasks."""
    self.task = task
    self.obj_ids = {'fixed': [], 'rigid': [], 'deformable': []}
    #p.resetSimulation(p.RESET_USE_DEFORMABLE_WORLD)
    ...
    # Reset task.
    self.task.reset_irr(self, obj_name)
    #self.task.reset(self)
  ...
  def insert_a_packing_object(self, obj_format_filename, target_location, color=None):
    # Move robot to home joint configuration.
    '''for i in range(len(self.joints)):
      p.resetJointState(self.ur5, self.joints[i], self.homej[i])
    ...
    self.task.add_an_object(self, obj_format_filename, target_location, color=color)
